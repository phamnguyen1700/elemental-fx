...
export interface ResolvedEffectArea {
  x: number;
  y: number;
  width: number;
  height: number;
...
function resolveOffset(size: number, alignment: EffectAreaAlignX | EffectAreaAlignY): number {
  if (alignment === "left" || alignment === "top") return 0;
  if (alignment === "right" || alignment === "bottom") return roundRatio(1 - size);
