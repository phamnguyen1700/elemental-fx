...
export function buildVineDistribution(
  ...
  /**
   * Secondary branches are part of the same biological vine system.
   *
   * VineGrowth already creates these branches; distribution is responsible for
   * giving them foliage accents after their structural skin has been emitted.
   */
  if (instances.length < maxInstances && vine.branches.length > 0) {
...
function buildStructuralSkin(
  ...
  /**
   * Secondary biological branches generated by VineGrowth.
   *
   * Previously these nodes existed in the physics topology but were not
   * rendered as branch assets, which made the visible structure read as a set
   * of independent cables instead of a climbing vine network.
   */
  for (const branch of vine.branches) {
    for (
      let segmentIndex = 0;
      segmentIndex < branch.nodes.length - 1;
      segmentIndex++
    ) {
      instances.push(
        createStructuralInstance(
...
function createStructuralInstance(
  ...
  if (!resource) {
    throw new Error("No branch resource is available.");
  }

  const baseCrossScale = lerp(
  ...
  const hierarchyRange =
    role === "secondary" ? config.secondaryBranchScale : config.mainBranchScale;

  const hierarchyScale = lerp(hierarchyRange[0], hierarchyRange[1], sample(14));

  const crossScale = baseCrossScale * hierarchyScale;
  ...
  return {
    ...
    flexibility:
      lerp(
        ...
        sample(3),
      ) * (role === "secondary" ? 1.12 : 1),

    flip: sample(4) < 0.5 ? -1 : 1,

    flutter:
      lerp(config.branchFlutter[0], config.branchFlutter[1], sample(5)) *
      (role === "secondary" ? 1.08 : 1),
